  ...
  <div v-loading="loading" element-loading-text="保存中...">
    <div class="publish-pics-list-wrap" v-if="publishVideoForm===true">
      <div class="publish-pics-list">
        <draggable :list="publishModal.images" :options="{handle: '.publish-pics-sort', draggable: '.publish-pics-list'}">
...
import {fetchArticleDetail, saveUpdateArticle, getRemoteArticleCategory} from '../../api/publish'
...
export default {
  ...
  mounted () {
    this.init()
    if (this.id) {
      fetchArticleDetail(this.id).then(result => {
  ...
  methods: {
    ...
    submitPics: function submitPics () {
      this.$refs.publishModal.validate(valid => {
        if (valid) {
          ...
          this.loading = true
          saveUpdateArticle(this.publishModal).then(result => {
    ...
    onSuccess (url, up, file) {
      ...
      if (this.publishModal.images.length) {
        for (let index in this.publishModal.images) {
