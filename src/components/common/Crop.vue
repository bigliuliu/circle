...
import {fetchUploadAvatar, fetchUploadImage} from '../../api/publish'
  ...
  methods: {
    ...
    cropImage () {
      ...
      if (this.imgUrl.indexOf('adva_') !== -1 && this.imgUrl.indexOf('_adva') !== -1) {
        ...
        fetchUploadAvatar(this.cropUrl + '&adva_' + this.s4() + this.s4(), ext).then(json => {
          if (json && json.code === 0) {
        ...
        const end = this.cropUrl.indexOf('?')
        const ext = this.cropUrl.substring(start, end)
        fetchUploadImage(this.cropUrl, ext).then(json => {
          if (json && json.code === 0) {
            const data = json.data
